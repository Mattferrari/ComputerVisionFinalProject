...
# Procesar la cámara de la Raspberry Pi en tiempo real
def procesar_camara_raspberry(color_ranges, picam):
    """
    Procesa la entrada de la cámara de la Raspberry Pi en tiempo real para detectar y seguir objetos de colores específicos.
    ...
    contadores = {color: 0 for color in color_ranges.keys()}
    tiempos_vuelta = {color: {} for color in color_ranges.keys()}
    dicc_tiempos = {color: time.time() for color in color_ranges.keys()}

    while True:
        frame = picam.capture_array()

        # Detectar la línea de meta
        meta_coords = detectar_meta(frame)

        # Detectar colores
        resultados = detectar_colores(frame, color_ranges)

        # Seguir objetos y verificar la línea de meta
        frame_resultado = seguir_objetos(frame, resultados, meta_coords, contadores, tiempos_vuelta, dicc_tiempos)

        # Mostrar resultado
        cv2.imshow('Seguimiento en tiempo real - Raspberry Pi', frame_resultado)
...
# Procesar la cámara de la Raspberry Pi en tiempo real
procesar_camara_raspberry(rangos_colores)'''